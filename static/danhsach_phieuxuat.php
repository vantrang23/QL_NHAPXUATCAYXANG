...
session_start();
include "connect.php";
$sql_dspx = "SELECT PX.ID_PHIEUXUAT, DATE_FORMAT(PX.THOIGIAN_XUAT, '%d-%m-%Y %H:%i:%s') as THOIGIAN_XUAT, NL.TEN_NHIENLIEU, NL.LOAI_NHIENLIEU, PX.SO_LUONG, PX.TONG_TIEN, BG.GIA, CONCAT(NV.HO_NV, ' ', NV.TEN_NV) AS FULLNAME
FROM PHIEUXUAT PX
JOIN NHANVIEN NV ON PX.ID_NV = NV.ID_NV
JOIN BANGGIA BG ON PX.ID_NHIENLIEU = BG.ID_NHIENLIEU
JOIN NHIENLIEU NL ON PX.ID_NHIENLIEU = NL.ID_NHIENLIEU
WHERE (BG.ID_NHIENLIEU, BG.NGAY_CN) IN (SELECT ID_NHIENLIEU, MAX(NGAY_CN) FROM banggia GROUP BY ID_NHIENLIEU)";
$result_dspx = mysqli_query($conn, $sql_dspx);
if (!$result_dspx) {
		...
		<?php
			...
			<main class="content">			
				<div class="container-fluid p-0">
					<h1 class="h3 mb-3" style="font-weight: bold;">DANH SÁCH PHIẾU XUẤT</h1>
									...
									<table id="dtBasicExample" class="table table-striped table-bordered table-sm" width="100%">
										<thead>
											<tr>
												...
												<th class="th-sm">Mã phiếu xuất</th>
										...
										<tbody>
											<?php
											$stt = 0;
											while ($row_dspx = mysqli_fetch_assoc($result_dspx)) {
												$stt++;
												$loai_nienlieu = ($row_dspx['LOAI_NHIENLIEU'] == 1) ? 'Xăng' : 'Dầu';
												echo "<tr>
														<td>{$stt}</td>
														<td>{$row_dspx['ID_PHIEUXUAT']}</td>
														<td>{$row_dspx['THOIGIAN_XUAT']}</td>
														<td>{$row_dspx['FULLNAME']}</td>
														<td>{$row_dspx['TEN_NHIENLIEU']}</td>
														<td>{$loai_nienlieu}</td>
														<td>{$row_dspx['SO_LUONG']}</td>
														<td>{$row_dspx['GIA']}</td>
														<td>{$row_dspx['TONG_TIEN']}</td>														
													</tr>";
											}
											?>
										</tbody>												  
