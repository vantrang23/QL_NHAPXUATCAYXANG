...
if (isset($_GET['id_phieunhap'])) {
	...
	$sql_pn = "SELECT PN.ID_PHIEUNHAP, PN.TRANG_THAI,DATE_FORMAT(PN.THOIGIAN_NHAP, '%d-%m-%Y %H:%i:%s') as THOIGIAN_NHAP,NCC.ID_NCC, NCC.TEN_NCC, CONCAT(NHANVIEN.HO_NV, ' ', NHANVIEN.TEN_NV) AS FULLNAME,PN.TONG_TIEN, TRAMXANGDAU.TEN_TRAM 
			FROM PHIEUNHAP PN
	...
	if(isset($_POST['nhapnhienlieu'])){
		...
		//SQL SERVER
		$sql_insert_ctpn_sqlrv = "INSERT INTO CTPN (ID_PHIEUNHAP,ID_NHIENLIEU,SO_LUONG,DONGIA) VALUES ('$id_phieunhap','$nhienlieu','$soluong', '$dongia')";
		$result_insert_ctpn_sqlsrv_=sqlsrv_query($conn_sv, $sql_insert_ctpn_sqlrv);
	...
	if (isset($_POST['danhaphang'])){
		...
		//SQL SERVER
		$sql_update_trangthai_sqlsrv="UPDATE PHIEUNHAP SET TRANG_THAI=1 WHERE ID_PHIEUNHAP='$id_phieunhap'";
		$result_update_trangthai_sqlsrv=sqlsrv_query($conn_sv, $sql_update_trangthai_sqlsrv);
		...
		while ($row_dutru = mysqli_fetch_assoc($result_dutru)) {
			...
			//SQL SERVER
			$sql_update_dutru_sqlsrv="UPDATE DUTRU SET SO_LUONG = SO_LUONG + {$row_dutru['SO_LUONG']} where ID_NHIENLIEU = {$row_dutru['ID_NHIENLIEU']} and ID_TRAM = '$id_tram'";
			$result_update_dutru_sqlsrv=sqlsrv_query($conn_sv, $sql_update_dutru_sqlsrv);
		}
									...
									<div class="col-12">									
										<div class="row justify-content-end">
											...
											<div class="col-md-3 ml-auto mb-3">					
												<button type="submit" name="nhapnhienlieu" class="btn btn-primary ml-auto mr-2">Add</button>
				...
				<?php }?>
												...
												$result_select_dsctpn = mysqli_query($conn, $sql_select_dsctpn);
													...
													if ($result_select_dsctpn && mysqli_num_rows($result_select_dsctpn) > 0) {
														...
														while ($row = mysqli_fetch_assoc($result_select_dsctpn)) {
															...
															<tr>
																	<td><?php echo $stt; ?></td>
																	<td><?php echo $row['TEN_NHIENLIEU']; ?></td>
																	<td><?php echo $loai_nhien_lieu; ?></td>
																	...
																	{}else{?>
																		<div class='btn-group ml-auto'>
																			<a href='xoa_ctpn.php?id_phieunhap=<?php echo $id_phieunhap; ?> &id_nhienlieu=<?php echo $row['ID_NHIENLIEU'];?>' class='btn btn-sm btn-outline-dark'><i class='fas fa-trash-alt'></i></a>
																		
																			<a href='update_ctpn.php?id_phieunhap=<?php echo $id_phieunhap; ?>&id_nhienlieu=<?php echo $row['ID_NHIENLIEU'];?>' class='btn btn-sm btn-outline-dark'><i class='fas fa-edit'></i></a>
																		</div>
																		<?php } ?> 
																	</td> 
					...
					<?php
						...
						if(isset($_POST['save'])){
							...
							//SQL SERVER
							$sql_update_tongtien_sqlsrv="UPDATE PHIEUNHAP  SET TONG_TIEN='$tongtien' where ID_PHIEUNHAP='$id_phieunhap' ";
							$result_update_tongtien_sqlsrv=sqlsrv_query($conn_sv, $sql_update_tongtien_sqlsrv);
					
					
								echo "<script>window.location='danhsach_phieunhap.php'</script>";
								// exit();					
